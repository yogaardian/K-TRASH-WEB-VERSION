    ...
    const fetchAllData = async () => {
      try {
        ...
        setBalanceData({
          ...
          hold_balance: balanceRes.data.hold_balance || 0,
    ...
    const interval = setInterval(() => {
      dashboardAPI.getUserBalance(userId)
        .then(res => setBalanceData({
          ...
          hold_balance: res.data.hold_balance || 0,
            ...
            <div className="saldo-card-top">
              ...
              <div>
                ...
                <h2 className="saldo-amount">
                  {isBalanceLoading ? "Memuat..." : `Rp ${Number(balanceData.total_balance).toLocaleString('id-ID')}`}
                </h2>
                {!isBalanceLoading && (
                  <p className="saldo-subtext" style={{ marginTop: 8, fontSize: '0.95rem' }}>
                    Saldo tersedia: Rp {Number(balanceData.available_balance).toLocaleString('id-ID')} • Saldo tertahan: Rp {Number(balanceData.hold_balance).toLocaleString('id-ID')}
                  </p>
                )}
              </div>
            </div>
