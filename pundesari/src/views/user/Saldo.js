  ...
  const fetchSaldoData = async () => {
    ...
    try {
      const [balanceRes, txRes] = await Promise.all([
        axios.get(`/user/balance/${userId}`),
        axios.get(`/user/transactions/${userId}`),
      ]);
            ...
            <Card.Body style={{ padding: 30 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap" }}>
                <div>
                  ...
                  <h2 style={{ margin: 0, fontSize: 36 }}>Rp {Number(balanceData.total_balance).toLocaleString('id-ID')}</h2>
                </div>
                <div style={{ textAlign: 'right' }}>
                  ...
                  <p style={{ margin: '8px 0 0', color: '#999' }}>Saldo tertahan: Rp {Number(balanceData.hold_balance).toLocaleString('id-ID')}</p>
                </div>
              </div>
            </Card.Body>
          </Card>
