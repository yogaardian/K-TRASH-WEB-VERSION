...
const StatusPenjemputan = () => {
  ...
  useEffect(() => {
    ...
    if (currentOrderId) {
      ...
      // Fetch order details
      axios
        .get(`http://localhost:3000/orders/${currentOrderId}`)
        .then((res) => {
          setOrder(res.data);
          // Fetch driver info jika ada
          if (res.data.driver_id) {
            return axios.get(`http://localhost:3000/drivers/${res.data.driver_id}`);
          }
        })
        .then((res) => {
          if (res) setDriver(res.data);
        })
