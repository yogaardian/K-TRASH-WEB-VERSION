...
function Saldo() {
  ...
  const fetchAdminData = async () => {
    setLoading(true);
    try {
      const [holdRes, pendingRes, historyRes, summaryRes] = await Promise.all([
        axios.get('/admin/settings/hold-balance'),
        axios.get('/admin/pending-transactions'),
        axios.get('/admin/transactions'),
        axios.get('/admin/hold-summary'),
      ]);

      const hold = Number(holdRes.data.minimum_hold_balance || 50000);
      setHoldAmount(hold);
      setHoldInput(hold);
      ...
      setFeedback({ type: 'danger', message: 'Gagal memuat data admin.' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleUpdateHold = async () => {
    try {
      ...
      await axios.patch('/admin/settings/hold-balance', { amount });
  ...
  const handleApprove = async (transactionId) => {
    try {
      await axios.patch(`/admin/approve-transaction/${transactionId}`, { admin_id: adminId || null });
  ...
  const handleReject = async (transactionId) => {
    try {
      await axios.patch(`/admin/reject-transaction/${transactionId}`, { admin_id: adminId || null });
